...
class LandUseRestriction(BaseModel):
    ...
    land_use_restriction_uri: Optional[StrictStr] = Field(
        default=None, alias="landUseRestrictionUri"
    )
    period_of_validity: TimePeriodDateOnly = Field(
        description="Voimassaoloaika", alias="periodOfValidity"
    )
    land_use_restriction_objects: Annotated[
        List[LandUseRestrictionObject], Field(min_length=1)
    ] = Field(
        description="Alueidenkäytön rajoituskohde", alias="landUseRestrictionObjects"
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "landUseRestrictionObjects": [
                    LandUseRestrictionObject.from_dict(_item)
                    for _item in obj["landUseRestrictionObjects"]
                ]
                if obj.get("landUseRestrictionObjects") is not None
                else None,
            }
        )
        return _obj
