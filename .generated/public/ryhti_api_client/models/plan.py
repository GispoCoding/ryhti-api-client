...
class Plan(BaseModel):
    ...
    partially_cancelled_plan_objects: Optional[
        List[PartiallyCancelledPlanObjectInfo]
    ] = Field(
        default=None,
        description="// Osittain kumottavat kaavakohteet",
        alias="partiallyCancelledPlanObjects",
    )
    plan_objects: Annotated[List[PlanObject], Field(min_length=1)] = Field(
        description="Kaavakohteet", alias="planObjects"
    )
    plan_regulation_groups: Annotated[
        List[PlanRegulationGroup], Field(min_length=1)
    ] = Field(description="Kaavakohteen määräysryhmät", alias="planRegulationGroups")
    ...
    ] = Field(default=None, alias="relatedPlanObjectRegulationGroupRelations")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in partially_cancelled_plan_objects (list)
        _items = []
        if self.partially_cancelled_plan_objects:
            for (
                _item_partially_cancelled_plan_objects
            ) in self.partially_cancelled_plan_objects:
                if _item_partially_cancelled_plan_objects:
                    _items.append(_item_partially_cancelled_plan_objects.to_dict())
            _dict["partiallyCancelledPlanObjects"] = _items
        ...
        # set to None if partially_cancelled_plan_objects (nullable) is None
        # and model_fields_set contains the field
        if (
            self.partially_cancelled_plan_objects is None
            and "partially_cancelled_plan_objects" in self.model_fields_set
        ):
            _dict["partiallyCancelledPlanObjects"] = None
        ...
        _obj = cls.model_validate(
            {
                ...
                "partiallyCancelledPlanObjects": [
                    PartiallyCancelledPlanObjectInfo.from_dict(_item)
                    for _item in obj["partiallyCancelledPlanObjects"]
                ]
                if obj.get("partiallyCancelledPlanObjects") is not None
                else None,
                "planObjects": [
