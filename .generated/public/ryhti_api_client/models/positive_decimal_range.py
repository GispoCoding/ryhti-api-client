...
class PositiveDecimalRange(AttributeValue):
    """
    PositiveDecimalRange
    """  # noqa: E501

    data_type: StrictStr = Field(
        description='Pakollinen arvo: "PositiveDecimalRange"', alias="dataType"
    )
    minimum_value: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="Minimiarvo", alias="minimumValue"
    )
    maximum_value: Optional[Union[StrictFloat, StrictInt]] = Field(
    ...
    def data_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["PositiveDecimalRange"]):
            raise ValueError("must be one of enum values ('PositiveDecimalRange')")
        return value
