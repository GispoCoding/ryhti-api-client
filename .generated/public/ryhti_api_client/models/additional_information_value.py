...
ADDITIONALINFORMATIONVALUE_ONE_OF_SCHEMAS = [
    "CodeValue",
    "DecimalRange",
    "DecimalValue",
    "IdentifierValue",
...
class AdditionalInformationValue(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in AdditionalInformationValue with oneOf schemas: CodeValue, DecimalRange, DecimalValue, IdentifierValue, LocalizedTextValue, NumericRange, NumericValue, PositiveDecimalRange, PositiveDecimalValue, PositiveNumericRange, PositiveNumericValue, SpotElevation, TextValue, TimePeriodDateOnlyValue, TimePeriodValue. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in AdditionalInformationValue with oneOf schemas: CodeValue, DecimalRange, DecimalValue, IdentifierValue, LocalizedTextValue, NumericRange, NumericValue, PositiveDecimalRange, PositiveDecimalValue, PositiveNumericRange, PositiveNumericValue, SpotElevation, TextValue, TimePeriodDateOnlyValue, TimePeriodValue. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
